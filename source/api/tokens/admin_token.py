...
async def generate_admin_token(body: GenerateAdminToken, token: str = Header(None)):
    try:
        ...
        if not validate_admin_access(
            token,
            local_base_worker,
            token_object_getter="get_main_admin_tokens_objects_list",
        ):
            raise HTTPException(
                status_code=403,
        ...
        return make_response(
            0, {"new_token": admin_token, "token_name": body.token_name}
        )
...
async def list_admin_token(token: str = Header(None)):
    try:
        ...
        return make_response(
            0, {"tokens": [i.prepare_for_return() for i in admin_token_list]}
        )
    except Exception as e:
        if not isinstance(e, HTTPException):
